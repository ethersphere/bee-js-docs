---
id: "utils.bytes"
title: "Namespace: Bytes"
sidebar_label: "Bytes"
custom_edit_url: null
hide_title: true
---

# Namespace: Bytes

[Utils](utils.md).Bytes

## Table of contents

### Interfaces

- [Bytes](../interfaces/utils.bytes.bytes.md)
- [FlexBytes](../interfaces/utils.bytes.flexbytes.md)

### Functions

- [bytesAtOffset](../functions/utils.bytes.bytesatoffset.md)
- [bytesEqual](../functions/utils.bytes.bytesequal.md)
- [flexBytesAtOffset](../functions/utils.bytes.flexbytesatoffset.md)
- [isBytes](../functions/utils.bytes.isbytes.md)
- [isFlexBytes](../functions/utils.bytes.isflexbytes.md)
- [makeBytes](../functions/utils.bytes.makebytes.md)
- [verifyBytes](../functions/utils.bytes.verifybytes.md)
- [verifyBytesAtOffset](../functions/utils.bytes.verifybytesatoffset.md)
- [verifyFlexBytes](../functions/utils.bytes.verifyflexbytes.md)
